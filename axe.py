...
Collaborative Mode:
  axe.py --collab llama,copilot --workspace ./playground --time 30 --task "Review code"
    ...
    # Check user namespace support if sandbox is enabled (show warning for all modes)
    ...
    if sandbox_enabled:
        ...
        user_ns_available = ns_supported
        if not ns_supported:
            print(c(f"⚠️  WARNING: {ns_message}", Colors.YELLOW))
            print(c("   Sandbox is enabled but user namespaces are not available.", Colors.YELLOW))
            print(c("   Using --unshare-user-try mode (no root privileges inside sandbox).", Colors.YELLOW))
            print(c("   File system isolation will still work correctly.", Colors.DIM))
            print(c("   This is common in containers, WSL, and some CI environments.", Colors.DIM))
            print()
