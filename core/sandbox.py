...
def check_user_namespace_support() -> Tuple[bool, str]:
    ...
    4. Try bwrap specifically with --unshare-user-try (most reliable)
    Returns:
    ...
    # Method 4: Test bwrap specifically with --unshare-user-try
    # This is the MOST reliable test since we actually use bwrap
    try:
        ...
        if bwrap_path:
            ...
            if result.returncode == 0:
                return True, "User namespaces fully supported (bwrap test passed)"
            else:
                stderr = result.stderr.strip() if result.stderr else "Unknown error"
                # Check for specific uid map error
                if 'setting up uid map' in stderr.lower() or 'permission denied' in stderr.lower():
                    return False, "Bubblewrap uid mapping unavailable (common in containers/CI)"
                return False, f"Bubblewrap test failed: {stderr}"
    except (subprocess.TimeoutExpired, FileNotFoundError, OSError) as e:
        pass  # Fall through to success if bwrap test couldn't run
    # If we got here, unshare worked but bwrap test was inconclusive
