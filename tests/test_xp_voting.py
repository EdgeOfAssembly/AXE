...
def test_self_vote_prevention():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # Use tmpdir directly as session_dir
        ws = GlobalWorkspace(tmpdir)
...
def test_workspace_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ws1 = GlobalWorkspace(tmpdir)
        ws1.vote_xp('@agent1', 10, '@agent2', 10, 'Good work')
        ws1.vote_xp('@agent2', 15, '@agent3', 8, 'Nice')
        
        # Load in new instance
        ws2 = GlobalWorkspace(tmpdir)
        votes = ws2.get_vote_history()
        
        assert len(votes) == 2, f"Should load 2 votes, got {len(votes)}"
        print("  ✓ Votes persisted to file")
        
        # Check vote limits persisted by reading workspace data
        data = ws2._read_workspace()
        assert '@agent1' in data['vote_limits']
        assert '@agent2' in data['vote_limits']
...
def test_broadcast_system():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Send broadcasts using correct signature: (agent_alias, agent_level, category, message, ...)
        bc1 = ws.broadcast('@agent1', 10, 'XP_VOTE', 'I voted for @agent2')
        bc2 = ws.broadcast('@agent2', 15, 'STATUS', 'Analysis complete')
        bc3 = ws.broadcast('@agent3', 5, 'XP_VOTE', 'Another vote')
        
        # Get all broadcasts
        all_bc = ws.get_broadcasts()
        assert len(all_bc) >= 3, f"Should have at least 3 broadcasts, got {len(all_bc)}"
        print("  ✓ Broadcasts stored")
        
        # Get filtered by category
        xp_bc = ws.get_broadcasts(category='XP_VOTE')
        assert len(xp_bc) >= 2, f"Should have at least 2 XP_VOTE broadcasts, got {len(xp_bc)}"
        print("  ✓ Broadcast filtering works")
        
        # Check ordering (most recent first)
        assert xp_bc[0]['agent'] == '@agent3', "Most recent should be first"
